...
On Windows, Podman runs in rootless mode by default, which does not assign a container IP. To enable proper networking, initialize Podman in rootful mode. Refer to the https://docs.podman.io/en/latest/markdown/podman-machine-init.1.html#rootful[official documentation^] for setup details.
