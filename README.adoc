...
On Windows, Podman runs in rootless mode by default, which does not assign a container IP. Since the `system` and `inventory` microservices need to communicate between containers, you must initialize Podman in https://docs.podman.io/en/latest/markdown/podman-machine-init.1.html#rootful[rootful^] mode to enable proper networking.
