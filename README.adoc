...
--
By default, Podman on Windows (WSL2) runs in rootless mode, which does not assign a container IP. To enable proper networking, Podman must be initialized in rootful mode.
...
--
No extra setup needed. Podman runs natively.
--
