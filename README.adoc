...
The `inventory` application WAR file [hotspot=inventory-war file=0]`inventory.war`, which was created from running Maven `package` goal, is copied to the [hotspot=config-apps file=0]`/config/apps` destination directory.
...
```
mvnw.cmd package
mvnw.cmd failsafe:integration-test -Dsystem.ip=[system-ip-address] -Dinventory.http.port=9081 -Dsystem.http.port=9080
```
--
...
```
./mvnw package
./mvnw failsafe:integration-test -Dsystem.ip=[system-ip-address] -Dinventory.http.port=9081 -Dsystem.http.port=9080
```
--
endif::[]
